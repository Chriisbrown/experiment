...
: fLXe_mt(NULL), fMPTPStyrene(NULL), fscint1_mt(NULL)
{
  ...
  fPstyrene = fPMMA = fPethylene1 = fPethylene2 = fscint1 = NULL;
...
void LXeDetectorConstruction::DefineMaterials(){
  ...
  //Scintillator1 Experiment
  fscint1 = new G4Material("Scintillator1", density= 1.023*g/cm3, 2);
  fscint1->AddElement(fC, 47.5*perCent);
  fscint1->AddElement(fH, 52.5*perCent);
  //Fiber(PMMA)
  ...
  G4double scint1_Energy[]    = { 7.0*eV , 7.07*eV, 7.14*eV };
  const G4int scint1num = sizeof(scint1_Energy)/sizeof(G4double);

  G4double scint1_SCINT[] = { 1.0, 1.0, 1.0 };
  assert(sizeof(scint1_SCINT) == sizeof(scint1_Energy));
  G4double scint1_RIND[]  = { 1.58 , 1.58, 1.58 };
  assert(sizeof(scint1_RIND) == sizeof(scint1_Energy));
  G4double scint1_ABSL[]  = { 380.*cm, 380.*cm, 380.*cm};
  assert(sizeof(scint1_ABSL) == sizeof(scint1_Energy));
  fscint1_mt = new G4MaterialPropertiesTable();
  fscint1_mt->AddProperty("FASTCOMPONENT", scint1_Energy, scint1_SCINT, scint1num);
  fscint1_mt->AddProperty("SLOWCOMPONENT", scint1_Energy, scint1_SCINT, scint1num);
  fscint1_mt->AddProperty("RINDEX",        scint1_Energy, scint1_RIND,  scint1num);
  fscint1_mt->AddProperty("ABSLENGTH",     scint1_Energy, scint1_ABSL,  scint1num);
  fscint1_mt->AddConstProperty("SCINTILLATIONYIELD",10000./MeV);
  fscint1_mt->AddConstProperty("RESOLUTIONSCALE",1.0);
  fscint1_mt->AddConstProperty("FASTTIMECONSTANT",0.9*ns);
  fscint1_mt->AddConstProperty("SLOWTIMECONSTANT",2.1*ns);
  fscint1_mt->AddConstProperty("YIELDRATIO",1.0);
  fscint1->SetMaterialPropertiesTable(fscint1_mt);

  // Set the Birks Constant for the LXe scintillator

  fscint1->GetIonisation()->SetBirksConstant(0.126*mm/MeV);
  ...
  G4double absorption1[]={20.*cm, 20.*cm, 20.*cm, 20.*cm};
...
void LXeDetectorConstruction::SetDefaults() {
  ...
  if(fLXe_mt)fLXe_mt->AddConstProperty("SCINTILLATIONYIELD",10000./MeV);
