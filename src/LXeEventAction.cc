...
LXeEventAction::LXeEventAction(LXeRecorderBase* r)
  : fRecorder(r),fSaveThreshold(0),fScintCollID(-1),fPMTCollID(-1),fVerbose(0),
   fPMTThreshold(1),fForcedrawphotons(false),fForcenophotons(true)
{
...
void LXeEventAction::EndOfEventAction(const G4Event* anEvent){
  ...
  if(scintHC){
    ...
    //man->FillNtupleDColumn(2,(eventInformation->GetEDep()));
    //man->AddNtupleRow();
    }
  G4AnalysisManager* man2 = G4AnalysisManager::Instance();
  man2->FillH1(2,(eventInformation->GetEDep()));
  }
  //G4AnalysisManager* man = G4AnalysisManager::Instance();
  G4AnalysisManager* man2 = G4AnalysisManager::Instance();
  G4int pmtshits[2];
  ...
  if(pmtHC){
    ...
    for(G4int i=0;i<pmts;i++){
      ...
      man2->FillH1((*pmtHC)[i]->GetPMTNumber(),((*pmtHC)[i]->GetPhotonCount()));
    ...
    man2->FillH1(3,ans);
    //man->FillNtupleDColumn(3,ans);
    //man->AddNtupleRow();
    if(fVerbose>0){
      G4cout << "\tRatio: " << ans << G4endl;
    }
